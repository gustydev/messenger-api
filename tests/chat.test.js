...
const { userRegister, userLogin, updateChat, createChat } = require('./requests');
...
beforeAll(async () => {
    ...
    const res3 = await createChat({ title: 'temporary title' }, 200, authorization) 
...
describe('new chat', () => {
    it('creates new chat with creator as admin', async () => {
        const res = await createChat({ title: 'Test Title' }, 200, authorization) 
    ...
    it('returns errors on invalid inputs', async () => {
        // Invalid: no title
        await createChat({ title: '' }, 400, authorization) 
        ...
        await createChat({ title: 'valid title', public: 'public' }, 400, authorization) 

        // invalid because title is not string
        await createChat({ title: true }, 400, authorization) 

        // invalid because description is not string
        await createChat({ title: 'title', description: 420 }, 400, authorization) 
    })
...
describe('update chat info', () => {
    it('updates chat with valid inputs', async () => {
        const res = await updateChat({
            ...
            public: true
        }, 200, updatedChat._id, authorization) 
    ...
    it('rejects invalid inputs when updating', async () => {
        await updateChat({
            title: '',
            public: 'no'
        }, 400, updatedChat._id, authorization) 
    })
